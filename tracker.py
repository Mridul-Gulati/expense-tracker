...
if user == "Rajvi" and password == "test123":
    ...
    if st.button("Add Transaction"):
        with st.spinner("Adding..."):
            try:
                client = gspread.service_account_from_dict({
                    "type": secrets["connections"]["gsheets_user2"]["type"],
                    "project_id": secrets["connections"]["gsheets_user2"]["project_id"],
                    "private_key_id": secrets["connections"]["gsheets_user2"]["private_key_id"],
                    "private_key": secrets["connections"]["gsheets_user2"]["private_key"],
                    "client_email": secrets["connections"]["gsheets_user2"]["client_email"],
                    "client_id": secrets["connections"]["gsheets_user2"]["client_id"],
                    "auth_uri": secrets["connections"]["gsheets_user2"]["auth_uri"],
                    "token_uri": secrets["connections"]["gsheets_user2"]["token_uri"],
                    "auth_provider_x509_cert_url": secrets["connections"]["gsheets_user2"]["auth_provider_x509_cert_url"],
                    "client_x509_cert_url": secrets["connections"]["gsheets_user2"]["client_x509_cert_url"]
                })
                spreadsheet_key = secrets["connections"]["gsheets_user2"]["spreadsheet"]
                worksheet_index = int(secrets["connections"]["gsheets_user2"]["worksheet"])
                sheet = client.open_by_key(spreadsheet_key).get_worksheet(worksheet_index)
            except Exception as e:
                st.error(f"An error occurred: {e}")
                st.stop()
